...
int main(int argc, char* argv[]) {
    ...
    if (cmd == Command::SEND) {
        ...
        try {
            ...
            std::cout << "Public endpoint: " << public_ep.ip << ":"
                      << public_ep.port << std::endl;
        ...
        std::cout << "Local endpoint: " << transfer.sender_local_ip << ":"
                  << transfer.sender_local_port << std::endl;

        // Register this transfer with rendezvous server
        Utils::register_transfer(transfer);
        std::cout << "Your secret is: " << transfer.id
        ...
        std::string host_override = std::getenv("ZAPSHARE_HOST_OVERRIDE")
                                        ? std::getenv("ZAPSHARE_HOST_OVERRIDE")
                                        : "";
